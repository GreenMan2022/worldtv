...
const translations = {
    ru: {
        ...
        "Смотрят": "Популярные", // ← переименовано
  ...
  en: {
    ...
    "Юэ китайский": "Cantonese",
    "Южноафриканский жестовый": "South African Sign Language",
    "Южный ндебеле": "Southern Ndebele",
    // === РЕГИОНЫ ===
    "Азия": "Asia",
    ...
    "Северная Америка (континент)": "North America (continent)",
...
// 👇 Дерево категорий (обновлено: "Смотрят" → "Популярные")
const categoryTree = {
  "Просмотренные": {},
  "Прямо сейчас": {},
...
// Обработчик клавиш (исправленный, с полной поддержкой всех стрелок)
document.addEventListener('keydown', function(e) {
    ...
    // 👇 КРИТИЧЕСКИ ВАЖНО: предотвратить прокрутку страницы стрелками ВСЕГДА
    ...
    switch(e.key) {
        ...
        case 'Enter':
            // ... (оставьте вашу текущую логику Enter без изменений)
            if (navigationState === 'mainCategories') {
                if (currentMainCategory === 'Просмотренные') {
                    navigationState = 'searchInput';
                    setTimeout(() => {
